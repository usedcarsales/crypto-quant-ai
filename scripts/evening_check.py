#!/usr/bin/env python3
"""
Evening Check — Paper Trading Pipeline
Same logic as afternoon_check.py, writes to evening memory file.
"""
...
def main():
    ...
    lines = []
    lines.append(f"# 📊 Paper Trading Evening Check — {now_et}")
    lines.append("")
    lines.append(f"- Cron ID: f890f449-22eb-4fbe-acf1-d78ddd4c4cef")
    ...
    lines.append("---")
    lines.append(f"*Evening check complete. Cron: f890f449*")

    report = "\n".join(lines)
    print(report)

    date_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    memory_path = f"/home/vinny2times/.openclaw/workspace/memory/{date_str}-paper-trading-evening.md"
