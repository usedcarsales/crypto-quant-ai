#!/usr/bin/env python3
"""
Batch 3 of 3: Coins 34-46 (last 13)
"""
...
for i, coin in enumerate(batch):
    ...
    try:
        ...
        print(f"[{i+35}/47] {sym}: TA={ta_score} RSI={ta_rsi}", flush=True)
    except Exception as e:
        print(f"[{i+35}/47] {sym}: {str(e)[:50]}", flush=True)
...
with open("/tmp/batch3.json", "w") as f:
    json.dump({"batch": 3, "results": results, "count": len(results)}, f)
print(f"Batch 3 done: {len(results)} coins processed")