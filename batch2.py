#!/usr/bin/env python3
"""
Batch 2 of 3: Coins 17-33 (middle)
"""
...
for i, coin in enumerate(batch):
    ...
    try:
        ...
        print(f"[{i+18}/47] {sym}: TA={ta_score} RSI={ta_rsi}", flush=True)
    except Exception as e:
        print(f"[{i+18}/47] {sym}: {str(e)[:50]}", flush=True)
...
with open("/tmp/batch2.json", "w") as f:
    json.dump({"batch": 2, "results": results, "count": len(results)}, f)
print(f"Batch 2 done: {len(results)} coins processed")